...
技能是指包含指令、脚本和资源的文件夹，Claude 会动态加载这些内容以提升在特定任务中的表现。技能教会 Claude 如何以可重复的方式完成具体任务，无论是按照公司品牌指南创建文档、使用组织特定的流程分析数据，还是自动化个人任务。
...
每个技能都独立存放在各自文件夹中，包含 `SKILL.md` 文件，其中包含 Claude 使用的指令和元数据。您可以浏览这些技能以获取创建自己技能的灵感，或了解不同的模式和 approaches。

此仓库中的许多技能都是开源的（Apache 2.0）。我们还包含了为 [Claude 的文档功能](https://www.anthropic.com/news/create-files) 提供支持的文档创建和编辑技能，位于 [`skills/docx`](./skills/docx)、[`skills/pdf`](./skills/pdf)、[`skills/pptx`](./skills/pptx) 和 [`skills/xlsx`](./skills/xlsx) 子文件夹中。这些是源码可用的，而非开源，但我们希望与开发者分享这些作为生产级 AI 应用中复杂技能的参考。

## 免责声明

**这些技能仅供演示和教育目的之用。** 虽然 Claude 可能提供其中部分功能，但您从 Claude 获得的实现和行为可能与这些技能中展示的内容不同。这些技能旨在说明模式和可能性。在将技能用于关键任务之前，请务必在您自己的环境中进行全面测试。
...
安装插件后，只需提及即可使用该技能。例如，如果你从市场安装了 `document-skills` 插件，可以让 Claude Code 执行类似这样的操作："使用 PDF 技能从 `path/to/some-file.pdf` 中提取表单字段"
...
你可以通过 Claude API 使用 Anthropic 的预置技能，并上传自定义技能。详情请参阅 [技能 API 快速入门](https://docs.claude.com/en/api/skills-guide#creating-a-skill)。

# 创建基础技能

技能的创建非常简单——只需一个包含 `SKILL.md` 文件的文件夹，其中包含 YAML 头部信息和说明。你可以使用本仓库中的 **template-skill** 作为起点：
...
技能是帮助 Claude 更好地使用特定软件的好方法。当我们发现合作伙伴提供的优秀技能示例时，可能会在此重点推荐：
